#ifndef OSMIUM_FILTERING_MULTIPOLYGON_MANAGER_HPP
#define OSMIUM_FILTERING_MULTIPOLYGON_MANAGER_HPP
...
class FilteringMultipolygonManager
    : public osmium::relations::RelationsManager<
          FilteringMultipolygonManager<TAssembler>, false, true, false> {
...
public:
    /**
     * Construct a FilteringMultipolygonManager.
     *
     * @param assembler_config The configuration that will be given to
     *                         any newly constructed area assembler.
     * @param filter A filter specifying what tags are needed on closed
     *               ways or multipolygon relations to build the area.
     * @param box A box for filtering out relations whose members do
     *            not intersect it.
     */
    template <typename Assembler, typename Filter, typename Box>
    explicit FilteringMultipolygonManager(Assembler && assembler_config,
                                          Filter && filter, Box && box)
        : m_assembler_config(std::forward<Assembler>(assembler_config))
        , m_filter(std::forward<Filter>(filter))
        , m_box(std::forward<Box>(box)) {}

    /**
     * Access the aggregated statistics generated by the assemblers
     * called from the manager.
     */
    const area_stats & stats() const noexcept { return m_stats; }
    ...
    bool new_relation(const osmium::Relation & relation) const {
        ...
        if(((!std::strcmp(type, "multipolygon")) ||
           ...
           osmium::tags::match_any_of(relation.tags(), m_filter)) {
            return std::any_of(
    ...
    void complete_relation(const osmium::Relation & relation) {
        ...
        if(std::none_of(ways.begin(), ways.end(), [&](const osmium::Way * way) {
               return boost::geometry::intersects(m_box,
                                                  m_factory.envelope(*way));
           }))
            return;
...
#endif  // OSMIUM_FILTERING_MULTIPOLYGON_MANAGER_HPP
